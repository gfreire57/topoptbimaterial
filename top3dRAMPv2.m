...
function top3dRAMP(nelx,nely,nelz,volfrac,penal,rmin)
...
Emin = 0.3;      % Young's modulus of void-like material
...
while change > tolx && loop < maxloop
    loop = loop+1;
    %% FE-ANALYSIS
    sK = reshape(KE(:)*(Emin+xPhys(:)'./(1+penal.*(1-xPhys(:)'))*(E0-Emin)), 24*24*nele, 1);
    K = sparse(iK,jK,sK); K = (K+K')/2;
    U(freedofs,:) = K(freedofs,freedofs)\F(freedofs,:);
    
    %% OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
    ce = reshape(sum((U(edofMat)*KE).*U(edofMat),2),[nely,nelx,nelz]);
    c = sum(sum(sum( (Emin+xPhys./(1+penal.*(1-xPhys))*(E0-Emin)).*ce )));
    dc = -(E0-Emin).*(1+penal)./((1+penal*(1-xPhys)).^2).*ce;
...
Top3dSTL_v3('ramp3d.stl', xPhys, 'Mode', 'iso')
clf; display_3D(xPhys); saveas(gcf,'Resultado.png');
end
